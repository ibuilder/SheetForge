...
| **No installer has been run on a *clean* machine** | The NSIS installer has now been run once, on the machine that built it: it installs per-user to `%LOCALAPPDATA%\Programs\SheetForge`, writes a working Start menu shortcut, ships an uninstaller, and the installed binary launches and renders. What that cannot tell anybody is what a stranger sees — the build machine trusts its own output, so SmartScreen's real behaviour on first run is still unobserved, as is uninstalling | 0.2 |
