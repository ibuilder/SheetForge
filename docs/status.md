...
| **The only published release cannot update itself** | [0.1.1](https://github.com/ibuilder/SheetForge/releases/tag/v0.1.1) is published, as a prerelease, and downloadable by anybody: all nine installers were fetched anonymously after publishing, and the attached `SHA256SUMS.txt` was checked by hashing the Windows installers independently. But it was built without `TAURI_SIGNING_PRIVATE_KEY`, so it carries no signed update payload and no `latest.json` — every copy installed from it is stranded on 0.1.1 until somebody uninstalls and reinstalls by hand. The first release built *with* the key is the first one that can fix anything after install | 0.2 |
