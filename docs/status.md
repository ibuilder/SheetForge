...
| **The only published release cannot update itself** | [0.1.1](https://github.com/ibuilder/SheetForge/releases/tag/v0.1.1) is published, as a prerelease, and downloadable by anybody: all nine installers were fetched anonymously after publishing, and the attached `SHA256SUMS.txt` was checked by hashing the Windows installers independently. But it was built without `TAURI_SIGNING_PRIVATE_KEY`, so it carries no signed update payload and no `latest.json` — every copy installed from it is stranded on 0.1.1 until somebody uninstalls and reinstalls by hand. **And the key alone would not change that.** The updater plugin is registered and configured with a public key and an endpoint, but nothing in the application ever calls it — no check on start, no menu item, and no `plugin:updater` call anywhere in the shipped bundle. A signed release published today would update nobody. Two things are needed, not one: the key, and code that asks. Every copy of 0.1.1 needs one manual reinstall regardless, because the asking code is not in it | 0.2 |
