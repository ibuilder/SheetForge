...
/// headers. This is the host half of
/// [ADR-0010](../../../../docs/adr/0010-page-assembly-produces-a-derived-revision.md): assembling
/// pages produces a **new revision** rather than editing the source, because a revision's identity
/// is the hash of its bytes and editing in place turns verification into a report of the user's
/// own work as tampering.
///
/// The origin is checked against the store rather than trusted. The interface names a revision it
/// believes it derived from, and a revision id that is not in this project is refused — a derived
/// document pointing at something that was never here is a provenance record that lies.
///
/// # Errors
/// [`CommandError::no_project`], [`CommandError::invalid_request`] if the origin is unknown or the
/// bytes are not a PDF, or a store error.
#[tauri::command]
pub async fn document_derive(
    app: AppHandle,
    ...
    let bytes = match request.body() {
        ...
        tauri::ipc::InvokeBody::Json(_) => {
            return Err(CommandError::invalid_request(
                "An assembled document must be sent as raw bytes.",
            ));
        }
    };

    let name = header(&request, "x-sf-name")?;
    let origin = header(&request, "x-sf-origin")?;
    let derivation = header(&request, "x-sf-derivation")?;
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        with_open(&state, |package| {
            ...
            let revision = DocumentRevision::derived(
                &from,
                document.id,
                hash,
                bytes.len() as u64,
            ...
            // Audited like an export, and for the same reason: this produces a document that
            // leaves the review, and "where did this come from?" is the question a dispute turns
            // on.
            audit(
                package,
                state.actor(),
...
/// A saved view, on the wire.
#[derive(Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ViewDto {
    /// What the reviewer called it. Also its identity within a document.
    pub name: String,
    /// 1-based page.
    pub page: u32,
    /// Magnification, 1.0 being actual size.
    pub zoom: f64,
    /// Centre of the view in PDF user space.
    pub center_x: f64,
    /// Centre of the view in PDF user space.
    pub center_y: f64,
    /// Page rotation in degrees.
    pub rotation: u32,
    /// The engine's markup filter, verbatim. Without it a restored view has the geometry and not
    /// the point.
    pub filter: Option<String>,
}

/// The saved views of one document.
///
/// # Errors
/// [`CommandError::no_project`], or a store error.
#[tauri::command]
pub fn view_list(app: AppHandle, revision: String) -> CommandResult<Vec<ViewDto>> {
    let state = app.state::<AppState>();
    state.require(Capability::ProjectRead)?;
    let id = revision_id(&revision)?;
...
/// Replace the saved views of one document.
///
/// Replace rather than add, because the engine holds the whole list and sends it whole. Merging
/// would make deletion impossible from the interface — a view the reviewer removed would come back
/// on the next save.
///
/// # Errors
/// [`CommandError::no_project`], or a store or domain error.
#[tauri::command]
pub fn view_replace(app: AppHandle, revision: String, views: Vec<ViewDto>) -> CommandResult<usize> {
    let state = app.state::<AppState>();
    state.require(Capability::MarkupCreate)?;
    let id = revision_id(&revision)?;
    ...
    with_open(&state, |package| {
        ...
        package.store_mut().replace_views(id, &records)?;
        Ok(written)
    })
}

/// What was filed, and how to ask for it back.
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct StoredAttachment {
    /// The content hash, in full. The interface refers to an attachment by this and by nothing
    /// else — it is not a path and cannot be turned into one.
    pub id: String,
    /// The first twelve characters, for anywhere a person has to read it.
    pub short_id: String,
    /// Size in bytes, so the interface can say how large a photo is before fetching it.
    pub byte_len: u64,
}

/// File an attachment — a site photo, a voice note, a scanned instruction.
///
/// Raw bytes, with the name and media type as headers. Content-addressed, so the same photo
/// attached to three markups is stored once.
///
/// The format is deliberately not sniffed. A source has to be a PDF because the renderer will be
/// asked to open it; an attachment is whatever somebody photographed on site, and refusing an
/// unfamiliar type would be refusing evidence. It is stored as bytes and handed back as bytes.
///
/// # Errors
/// [`CommandError::no_project`], or a package error if it is past the attachment limit.
#[tauri::command]
pub async fn attachment_store(
    app: AppHandle,
    request: tauri::ipc::Request<'_>,
) -> CommandResult<StoredAttachment> {
    let bytes = match request.body() {
        tauri::ipc::InvokeBody::Raw(bytes) => bytes.clone(),
        tauri::ipc::InvokeBody::Json(_) => {
            return Err(CommandError::invalid_request(
                "An attachment must be sent as raw bytes.",
            ));
        }
    };
    let name = header(&request, "x-sf-name")?;
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        with_open(&state, |package| {
            ...
            let hash = package.import_attachment(&bytes)?;

            // Audited: an attachment is evidence, and when it entered the project is part of what
            // makes it evidence. The name is not recorded — it is document content, and the rule
            // is that none of that reaches a log.
            audit(
                package,
                state.actor(),
                "attachment:store",
                Outcome::Allowed,
                Record::new()
                    .subject("attachment", &hash.to_hex())
                    .with("bytes", &byte_len.to_string()),
            );
...
/// An attachment's bytes, for the interface to display or save.
///
/// A raw IPC response rather than JSON, for the same reason as a drawing: a 12 MB photo serialised
/// as an array of numbers is 60 MB of text to build, send and parse.
///
/// The stored bytes are re-hashed on the way out. A photo is evidence of a defect, and evidence is
/// exactly what somebody later claims was altered.
///
/// # Errors
/// [`CommandError::no_project`], or a package error if it is missing or altered.
#[tauri::command]
pub async fn attachment_bytes(app: AppHandle, id: String) -> CommandResult<tauri::ipc::Response> {
    tauri::async_runtime::spawn_blocking(move || {
        let state = app.state::<AppState>();
        state.require(Capability::ProjectRead)?;

        let hash = sf_domain::ContentHash::from_hex(&id)
            .map_err(|_| CommandError::invalid_request("That is not an attachment."))?;

        with_open(&state, |package| {
            let bytes = package.attachment_bytes(hash)?;
...
/// Check a measured dimension against the one printed on the sheet.
///
/// Pure arithmetic, and it lives on the host rather than in the interface because where the bands
/// sit — agrees, close, wrong — is a domain judgement with tests behind it. A tolerance that lives
/// in a click handler is a tolerance the importer will not obey, and one somebody widens on the
/// afternoon it becomes inconvenient.
///
/// Both lengths must already be in the same unit. This is arithmetic about magnitudes, not a unit
/// converter.
///
/// # Errors
/// [`CommandError::invalid_request`] if either length is not a positive, finite number.
#[tauri::command]
pub fn scale_check(expected: f64, measured: f64) -> CommandResult<sf_domain::ScaleCheck> {
