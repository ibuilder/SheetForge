...
SheetForge opens a tutorial drawing the first time you start it — a two-page sheet with a title
block, a column grid, a legend and a graphic scale, which you can mark up and measure freely. Page
two carries a dimension printed as `144'-0"`. Calibrate against it, measure the other side, and the
sheet tells you whether you got it right: it should read `96'-0"`.

That is the habit the tool is built around. A takeoff number is only worth what its scale is worth,
so the first thing the application teaches is how to check one.

It opens itself once. After that it is under **Project → Open the tutorial sheet**, and on the
empty screen. It lives in `Documents/SheetForge/SheetForge Tutorial.sfproj` like any other project;
delete that folder and it is gone.

![Sheet A-201 of the tutorial drawing: a floor plan on a lettered and numbered column grid, with an
overall dimension reading 144'-0" beneath it and 96'-0" down the left-hand side, a room marked OPEN
OFFICE at 2,304 SF, a graphic scale and a title block.](docs/assets/tutorial-sheet-a201.png)

*The practice sheet. It is generated by [`scripts/make-welcome-sheet.mjs`](scripts/make-welcome-sheet.mjs)
and reviewed as code — synthetic on purpose, because a real construction drawing is somebody's
property and does not belong in a repository.*
